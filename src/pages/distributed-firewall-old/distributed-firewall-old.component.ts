...
import { FirewallPolicy, FirewallRule, Service } from '../distributed-firewall/types';
...
import { EditableNameComponent } from '../gateway-firewall/components/editable-name/editable-name.component';
import { ActionDropdownComponent } from '../gateway-firewall/components/action-dropdown/action-dropdown.component';
...
@Component({
  ...
  template: `
    <div class="rounded-xl p-6 bg-white dark:bg-slate-800 shadow space-y-4">
      ...
      <div class="space-y-3">
        <div>
          <h1 class="text-2xl font-bold text-[#293c51] dark:text-gray-200">Distributed Firewall (Legacy)</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage network security policies and rules across your infrastructure.</p>
        </div>
            ...
            @for (policy of filteredPolicies(); track policy.id) {
              ...
              <tr>
                <td colspan="8" class="p-12 text-center">
                  ...
                  <h3 class="text-lg font-semibold text-[#293c51] dark:text-gray-200">No Firewall Policies</h3>
                  <p class="mt-2 text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto">You haven't created any firewall policies yet. Click "Add Policy" to get started.</p>
...
export class DistributedFirewallOldComponent {
  ...
  // Main state
  policies = this.dfwService.policies;
  ...
  togglePolicy(policyId: string): void {
    this.dfwService.updatePolicy(policyId, { isExpanded: !this.dfwService.getPolicyById(policyId)?.isExpanded });
  }
  ...
  handleConfirmDelete(): void {
    ...
    selected.forEach(id => {
        ...
        this.dfwService.deletePolicy(id);
    });
  ...
  handlePolicyNameSave(policyId: string, newName: string): void {
      this.dfwService.updatePolicy(policyId, { name: newName });
  }

  handleRuleNameSave(policyId: string, ruleId: string, newName: string): void {
      this.dfwService.updateRule(policyId, ruleId, { name: newName });
  }

  handleRuleActionChange(policyId: string, ruleId: string, newAction: 'Allow' | 'Deny' | 'Drop' | 'Reject'): void {
    this.dfwService.updateRule(policyId, ruleId, { action: newAction });
  }

  handleRuleUpdate(policyId: string, ruleId: string, field: keyof Omit<FirewallRule, 'id' | 'ruleId'>, value: any): void {
    this.dfwService.updateRule(policyId, ruleId, { [field]: value });
  }
  ...
  handleAddRuleClick(): void {
    // Logic to add a rule to the selected policy
    console.log('Adding rule to selected policy');
  }
  ...
  handleConfirmDeletePolicy(): void {
    ...
    if (policy) {
        this.dfwService.deletePolicy(policy.id);
    }
